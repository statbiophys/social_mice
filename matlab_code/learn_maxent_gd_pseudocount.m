...
mirExp_bsstd(mirExp_bsstd<0.01) = 0.005; % add some threshold...
% need to see if std of j stabilize or not. hope it does!
%%
...
%     if (cerror<cthres/3)&&(merror<mthres)
...
%         % this have some problems: if signal = 1 all the time, i.e. if the
%         % mouse have not moved, we willl have mirExp_bsstd = 0.
%         % so we need to replace the stopping criterion.
%         mcFlag=0;
%     end
...
%             abs(stdj2(end) - stdj2(end-100)) < 0.005
%             mcFlag = 0;
%         end
%     end
    ...
    % mean jij and std jij doesn't change much
    % Change much is how much?
    % Need to be less than what random halves give
    % Will come back to this
    % Now set to 0.01 in 50 time steps?
